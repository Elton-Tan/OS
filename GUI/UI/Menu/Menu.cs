...
namespace TangerineOS.GUI
{
    internal class Menu : Process
    {
        ...
        internal override int Start()
            //show your applications in the startbar
        {
            appList.Clear();
            ...
            Font.DrawImageAlpha(Icons.lockicon, 10, 220, menuBitmap.rawData, 249); //create Lock icon
            Font.DrawString("Lock", -1, 40, 225, menuBitmap.rawData, 249); //create the text 'Lock'
        ...
        internal override void Update()
        {
            ...
            GUI.canvas.DrawString(TangerineOS.Date.CurrentTime(true), GUI.font, GUI.WhitePen, (int)GUI.screenX - 70, (int)GUI.screenY - 20); //show time on the bottom right
            if (Kernel.useNetwork) { GUI.canvas.DrawImageAlpha(Icons.connected, (int)GUI.screenX - 100, (int)GUI.screenY - 26); } //create the neytwork icon at the bottom right
            ...
            if (MouseManager.Y > GUI.screenY - 30)
            {
                ...
                else if (MouseManager.X > (int)GUI.screenX - 75)
                {
                    ...
                    else { GUI.canvas.DrawString(TangerineOS.Date.CurrentDate(true, false), GUI.font, GUI.WhitePen, (int)GUI.screenX - 200, (int)GUI.screenY - 45); } //show date
                }
                else if (Kernel.useNetwork && MouseManager.X > (int)GUI.screenX - 100)
                {
                    if (!Opened) { GUI.canvas.DrawImageAlpha(start, 5, (int)GUI.screenY - 27); }
                    if (GUI.Pressed) { }
                    else { GUI.canvas.DrawString(TangerineOS.Net.GetInfo(), GUI.font, GUI.WhitePen, (int)GUI.screenX - 200, (int)GUI.screenY - 45); }
                }
                else
                {
                    if (!Opened) { GUI.canvas.DrawImageAlpha(start, 5, (int)GUI.screenY - 27); }
