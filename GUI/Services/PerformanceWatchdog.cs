namespace TangerineOS.GUI
{
    internal class PerformanceWatchdog : Process
    {
        ...
        {
            if (TangerineOS.Sysinfo.InstalledRAM < 250) { Toast.msg = "Installed system memory is lower than the recommended 256MB. System stability may be impacted."; } //to do - notification system
            return 0;
        }
        internal override void Update()
        {
            if ((int)(TangerineOS.Sysinfo.UsedRAM / TangerineOS.Sysinfo.InstalledRAM * 100) > 79)
            {
                if (!acknowledged1)
                {
                ...
                if ((int)(TangerineOS.Sysinfo.UsedRAM / TangerineOS.Sysinfo.InstalledRAM * 100) > 89)
                {
                    if (!acknowledged2)
                    {
                    ...
                    if ((int)(TangerineOS.Sysinfo.UsedRAM / TangerineOS.Sysinfo.InstalledRAM * 100) > 94)
                    {
                        if (!acknowledged3)
                        {
